...
const Header = () => {
  ...
  const [mobileMenuData, setMobileMenuData] = useState<any[]>([]);
    ...
    if (!hasMounted.current) {
      ...
      const fetchData = async () => {
        try {
          ...
          setMobileMenuData(data?.mobileHeaderData);
        } catch (error) {
    ...
    <>
      <header className={`fixed top-0 left-0 z-50 w-full`}>
        <div className='container mx-auto p-3'>
          <nav
            className={`flex w-full items-center py-3 px-4 justify-between ${sticky
            ...
            <div className='flex items-center gap-1 xl:gap-4'>
              ...
              {user?.user || session?.user ? (
                <div className='hidden lg:flex gap-4'>
                  ...
                  <div className='flex items-center text-sm font-medium text-dark_black dark:text-white capitalize px-3 py-2 bg-dark_black/5 dark:bg-white/5 rounded-full'>
        ...
        <div
          ...
          <div className='p-4'>
            <ul className='flex flex-col'>
              {mobileMenuData && mobileMenuData?.map((item, index) => (
              ...
              <div className='flex flex-col items-center gap-3 px-2 mt-2'>
                {user || session?.user ? (
                  <>
                    <button
                      onClick={() => signOut()}
                      className='flex w-full group font-normal items-center gap-2 transition-all duration-200 ease-in-out text-white dark:text-white px-4 py-2 bg-dark_black dark:bg-white/10 rounded-full hover:text-dark_black hover:bg-white dark:hover:text-dark_black border border-dark_black dark:border-white/10'>
                      Sign Out
                      <Icon
                    ...
                    <div className='group flex gap-2 items-center w-full border border-dark_black dark:border-white px-4 py-2 rounded-md hover:bg-dark_black transition-all duration-200 ease-in-out'>
                      <p className='group-hover:text-white text-dark_black dark:text-white w-full capitalize text-center font-medium'>
                        {user?.user || session?.user?.name}
                      </p>
                    </div>
                  </>
                  ...
                  <>
                    <Link
                      href={'/signin'}
                      className='w-full border border-dark_black dark:border-white text-primary px-4 py-2 rounded-full hover:bg-dark_black dark:hover:bg-white hover:text-white dark:hover:text-dark_black'>
                      Sign In
                    </Link>
                    <Link
                      href={'/signup'}
                      className='w-full text-white dark:text-dark_black px-4 py-2 bg-dark_black dark:bg-white rounded-full hover:opacity-90'>
                      Sign Up
                    </Link>
                  </>
                )}
